...
`AppIcon-Placeholder-1024.png`, in both the iOS and Watch asset catalogs,
generated by `Tools/make_placeholder_icon.py` and regenerable at any time by
running it. A sky wash in the accent sage with a plain sun disc — **no character
is drawn in it**, deliberately. `CLAUDE.md` forbids inventing a Sunnie design,
and an icon containing no Sunnie cannot accidentally become one.

It exists because App Store Connect refuses an upload from a bundle with no icon,
which put it on the critical path to TestFlight (`Documentation/TESTFLIGHT_SETUP.md`).
Replace it with the real mark when there is one; nothing depends on its content.
...
| App icon | Placeholder, generated | `Assets.xcassets/AppIcon` (iOS and Watch) |
